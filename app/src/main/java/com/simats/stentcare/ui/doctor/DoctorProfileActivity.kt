...
class DoctorProfileActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        ...
        // Edit Profile (null-safe — won't crash if card not in layout)
    ...
    private fun loadProfile() {
        tvName.text = "Dr. ${DJStentCareApp.instance.getUserName() ?: "User"}"
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body()?.success == true) {
                    ...
                    tvName.text           = "Dr. ${p.fullName}"
                    tvEmail.text          = p.email ?: "-"
                    tvPhone.text          = p.phone ?: "-"
                    tvHospital.text       = p.hospitalName ?: "-"
                    tvSpecialization.text = p.specialization ?: "Urologist"
                    tvTotalPatients.text  = p.totalPatients.toString()
                    tvActiveStents.text   = p.activeStents.toString()
    ...
    private fun showEditProfileDialog() {
        ...
        etName.setText(tvName.text.toString().removePrefix("Dr. "))
            ...
            try {
                ...
                if (response.isSuccessful && response.body()?.success == true) {
                    ...
                    DJStentCareApp.instance.saveUserSession(
                        ...
                        role   = DJStentCareApp.instance.getUserRole() ?: "doctor",
                        name   = name
                    )
                    loadProfile()
                } else {
                    ...
                    Toast.makeText(this@DoctorProfileActivity, errMsg, Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
    ...
    private fun changePassword(oldPwd: String, newPwd: String) {
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body()?.success == true) {
                    Toast.makeText(this@DoctorProfileActivity, "Password changed successfully!", Toast.LENGTH_SHORT).show()
                } else {
                    val errMsg = try {
